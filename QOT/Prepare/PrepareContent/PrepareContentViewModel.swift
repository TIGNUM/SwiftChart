...
enum PrepareContentItem {
    ...
    case collection(localID: String, title: String, items: [PrepareSubContentItem])
}

enum PrepareSubContentItem {
...
private func mockContentItems() -> [PrepareContentItem] {
    return [
        .title(localID: UUID().uuidString, text: "SUSTAINABLE HIGH PERFORMANCE TRAVEL"),

        .collection(localID: UUID().uuidString, title: "PRE TRAVEL", items: mockContentSubItems()),

        .collection(localID: UUID().uuidString, title: "ON TRAVEL", items: mockContentSubItems()),

        .collection(localID: UUID().uuidString, title: "AFTER TRAVEL", items: mockContentSubItems()),
        ...
        .text(localID: UUID().uuidString, text: "There are many remedies and strategies out there (especially on the internet) but many of them are unproven, impractical, or unsafe. The following suggestions are the best strategies that have worked for our TIGNUM clients who travel millions of kilometers/miles every year. These suggestions follow our totally integrated approach (Mindset, Nutrition, Movement, and Recovery) and address all of the major organ systems."),

        .text(localID: UUID().uuidString, text: "At TIGNUM we believe in a new level of preparation (this requires planning) and therefore, we look at Sustainable High Performance Travel from a pre-travel, during travel, and post-travel perspective.")
    ]
}

private func mockContentSubItems() -> [PrepareSubContentItem] {
    var items: [PrepareSubContentItem] = []
    for i in 1...10 {
