        ...
        if let finalAnswers = answers {
            ...
            if answerIndex < finalAnswers.count {
                ...
                case .customize:
                    ...
                    ThemeText.questionHintLabel.apply(finalAnswers[answerIndex].title?.capitalizingFirstLetter(), to: hintLabel)
                default:
                    indexLabel.attributedText = formTimeAttibutedString(title: finalAnswers[answerIndex].subtitle ?? "", isLast: answerIndex == finalAnswers.count - 1)
                    ThemeText.questionHintLabelDark.apply(finalAnswers[answerIndex].title?.capitalizingFirstLetter(), to: hintLabel)
                 }
            }
        } else {
            ...
            ThemeText.questionHintLabel.apply(subtitles[items - index - 1].capitalizingFirstLetter(), to: hintLabel)
        }
