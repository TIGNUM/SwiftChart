...
extension LaunchHandler {
    ...
    func showPreparationWith(identifier: String) {
        UserService.main.getUserPreparationWith(qotId: identifier) { (preparation, _, _) in
    ...
    func showBookmarks() {
        ...
        let item = MyLibraryCategoryListModel(title: String.empty, itemCount: 0, lastUpdated: Date(), icon: nil, type: .BOOKMARK, newItemCount: 0)
    ...
    func showDownloads() {
        ...
        let configurator = MyLibraryUserStorageConfigurator.make(with: nil)
        let item = MyLibraryCategoryListModel(title: String.empty, itemCount: 0, lastUpdated: Date(), icon: nil, type: .DOWNLOAD, newItemCount: 0)
        configurator(controller, item)
        push(viewController: controller)
    }
