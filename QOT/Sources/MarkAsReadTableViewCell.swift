...
final class MarkAsReadTableViewCell: UITableViewCell, Dequeueable {
    ...
    func configure(selected: Bool) {
        isRead = selected
        let text = isRead ? AppTextService.get(.know_strategy_list_strategy_section_body_button_mark_as_unread) :
                            AppTextService.get(.know_strategy_list_strategy_section_body_button_mark_as_read)
