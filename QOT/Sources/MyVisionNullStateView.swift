...
final class MyVisionNullStateView: UIView {
    ...
    func setupView(with header: String, message: String, delegate: MyVisionNullStateViewProtocol?) {
        ...
        ThemeText.tbvSectionHeader.apply(AppTextService.get(AppTextKey.my_qot_tbv_null_state_view_title), to: toBeVisionLabel)

        writeButton.setAttributedTitle(ThemeText.tbvButton.attributedString(AppTextService.get(AppTextKey.my_qot_tbv_empty_write_button)), for: .normal)
        autoGenerateButton.setAttributedTitle(ThemeText.tbvButton.attributedString(AppTextService.get(AppTextKey.my_qot_tbv_empty_auto_generate_button)), for: .normal)
