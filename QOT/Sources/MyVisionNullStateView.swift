...
final class MyVisionNullStateView: UIView {
    ...
    func setupView(with header: String, message: String, writeMessage: String, delegate: MyVisionNullStateViewProtocol?) {
        ...
        let buttonTitle = ThemeText.tbvButton.attributedString(AppTextService.get(.my_qot_my_tbv_null_state_button_auto_generate))
        autoGenerateButton.setAttributedTitle(buttonTitle, for: .normal)
        ThemeBorder.white.apply(writeButton)
        ThemeBorder.white.apply(autoGenerateButton)
