...
extension UIViewController {
        ...
        if let networkError = error as? QOTNetworkError {
            ...
            case .unknown(let error, let statusCode):
                showAlert(messageType: String(format: "error: %@\nStatusCode: %d",
                                               error?.localizedDescription ?? String.empty, statusCode ?? 0))
    ...
    func showAlert(messageType: String) {
        ...
        let message = AppTextService.get(.generic_alert_unknown_error_new_body).replacingOccurrences(of: "${MESSAGE_TYPE}", with: messageType)
    ...
    func createCloseButton(_ action: Selector) -> UIBarButtonItem {
         ...
         let heightConstraint = NSLayoutConstraint.init(item: button,
                                                        attribute: .height,
                                                        relatedBy: .equal,
                                                        toItem: nil,
                                                        ...
                                                        multiplier: 1,
                                                        constant: 40)
         let widthConstraint = NSLayoutConstraint.init(item: button,
                                                       attribute: .width,
                                                       relatedBy: .equal,
                                                       toItem: nil,
                                                       attribute: .notAnAttribute,
